...
const CELL_BG: Record<BadgeTone, (intensity: number) => string> = {
  ...
  accent: (i) => (i === 0 ? "bg-muted/30" : i < 0.5 ? "bg-status-not-started/15" : "bg-status-not-started/30"),
};
        ...
        <div className="p-3">
          ...
          >
            ...
            {matrix.rows.map((row, idx) => (
              <FragmentRow key={row.key} row={row} colMax={colMax} showArrow={idx > 0 && idx < matrix.rows.length - 1} />
            ))}
          </div>
        </div>
      </div>
...
function FragmentRow({
  ...
  return (
    <>
      <div className="relative flex items-center pr-2 text-xs font-semibold text-foreground/85">
        {row.label}
        {showArrow && (
          <ArrowDown className="absolute -bottom-3 left-1/2 -translate-x-1/2 h-3 w-3 text-muted-foreground/40" aria-hidden />
        )}
      </div>
